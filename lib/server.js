...
module.exports = function (sbot, listenAddr, onEnd) {
  ...
  function serveRepoTree(repo, branch, path) {
    return renderRepoPage(repo, branch, cat([
      renderRepoTree(repo, branch, path),
      renderRepoReadme(repo, branch, path),
    ]))
  }
  ...
  function renderRepoTree(repo, rev, path) {
    ...
    return cat([
      pull.once('<h3>' + type + ': ' + escapeHTML(rev) + '</h3><ul>'),
      pull(
        repo.readTree(rev),
        pull.map(function (file) {
          var path = [repo.id, 'blob', rev, file.name]
