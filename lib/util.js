...
u.getRemoteUrl = function (name) {
  if (!name) return
  return u.gitSync('remote', 'get-url', name)
}

u.getCurrentBranch = function () {
  return u.gitSync('describe', '--contains', '--all', 'HEAD')
}
...
u.editor = function (name, defaultText, cb) {
  var fs = require('fs')
  ...
  child.on('exit', function (code) {
    ...
    fs.readFile(filename, 'utf8', function (err, text) {
      ...
      fs.unlink(filename, function (err) {
        cb(err, text.split(separator)[0].trim())
      })
    })
  })
}
